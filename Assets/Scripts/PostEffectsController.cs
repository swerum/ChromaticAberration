...
public class PostEffectsController : MonoBehaviour
{
    ...
    [SerializeField] Vector2 redBaseOffset = new Vector2(0.004f, 0);
    [SerializeField] Vector2 blueBaseOffset = new Vector2(0.004f, 0);
    Material postEffectMaterial;

    private void Awake() {
        if (postEffectMaterial == null) { postEffectMaterial = new Material(postShader); }
        SetAberrationOffsets(Vector2.zero, Vector2.zero);
    }
